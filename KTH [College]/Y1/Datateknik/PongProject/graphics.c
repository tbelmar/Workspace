...
void drawPaddle(Paddle paddle) {
	displayCords(paddle.x, paddle.y);
}

void drawBall(Ball theBall) {
	drawPixels(theBall.x, theBall.y);
}

void drawScore(Paddle p1, Paddle p2) {
	int i, x = 52;
	
	player_1.points = player_1.points % 128;
	player_2.points = player_2.points % 128;
	char c = player_1.points + '0';

	for (i = 0; i < 8; i++) {
		pixelArray[x + i] = font[c * 8 + i];
	}
